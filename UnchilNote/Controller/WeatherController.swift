...
class WeatherController: ObservableObject {
	...
	func setListWeather(context: NSManagedObjectContext) {
		...
		if let location = locationService.cLLocationManager.location {
			let openWeatherURL =
			 "\(ConstVar.OpenWeatherURL)?lat=\(location.coordinate.latitude)&lon=\(location.coordinate.longitude)&appid=\(ConstVar.OpenWeatherSdkApiKey)&units=\(ConstVar.OpenWeatherUnits)"
